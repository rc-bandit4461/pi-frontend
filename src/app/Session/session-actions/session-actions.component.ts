...
import {Etudiant, EtudiantSession, Filiere, Element, Module, NoteEtudiant, SemestreFiliere, Session, Examen} from '../../entities/entities';
...
export class SessionActionsComponent implements OnInit {
  ...
  private isAllDataGathered: boolean = false;
  private noteEtudiants: NoteEtudiant[];
  ...
  async loadSemestreFilieres() {
    if(this.isAllDataGathered) return;
    try {
    let data;
  ...
  onClickEtudiant(etudiant: Etudiant) {
